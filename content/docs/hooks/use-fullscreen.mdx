...
```bash
npx shadcn@latest add "https://h3-use.com/r/use-fullscreen"
```
...
- Call `enter()` to make the element fullscreen.
- Call `exit()` to exit fullscreen mode.
- `isFullscreen` will be `true` if the element is currently fullscreen.

If you pass `null` as the ref, the entire page will enter fullscreen mode.
